...
export function NonVegPizzas() {
  ...
  let [nonVegPizzas, setNonVegPizzas] = useState([]);
  ...
  let [showMessage,setShowMessage] = useState(false);
   ...
   const fetchNonVegPizzas = () => {
    fetch(`${API}/items/getNonVegPizzas`)
    .then((res) => res.json())
    .then((res) => {
      setShowSpinner(false);
      setNonVegPizzas(res.result);
    })
  }

  useEffect(fetchNonVegPizzas,[]);
  ...
  const deleteItem = async(id) => {
    try{
      await axios.delete(`${API}/delete/deleteNonVegPizza/${id}`);
      fetchNonVegPizzas()
    }
    catch(err){
  ...
  return (
    <div className="container mt-3">
      <h2 className="pt-2">Non-Veg Pizzas</h2>
                  ...
                  <div className="d-flex flex-column size-and-crust gap-2">
                    <div className="d-flex gap-2">
                      ...
                      {priceKeys.map((key, index) => {
                        return (
                          <div className="form-check d-flex gap-1" key={index}>
                            <input
                    ...
                    <div className="d-flex gap-3">
                      <div className="d-flex flex-column w-50">
                        <label className="crust-label text-danger text-start">
                          Crust
                        </label>
                        <select
                          className="form-select crust-selection"
                          id="crustSelection"
                      ...
                      <div className="d-flex flex-column w-25">
                        <label className="qty-label text-danger text-start">
                          Qty
                        </label>
                        <input
                          type="number"
                      ...
                      onClick={() => {
                        if (!userLogged) {
                          ...
                          if (selectedPizzaSize !== "" && qty >= 1) {
                            ...
                            selectedPizzaObj.price =
                              priceObj[selectedPizzaSize];
                            selectedPizzaObj.crust = selectedCrust;
                            selectedPizzaObj.quantity = qty;
                            selectedPizzaObj.finalPrice = getCrustPrice(
                              selectedCrust,
                              selectedPizzaSize,
                              qty
                            );
